...
// Asegúrate de que esta imagen exista o comenta la línea si no la tienes aún
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between h-full max-w-8xl mx-auto">
            ...
            <div className="flex items-center gap-2">
                ...
                <Link to="/pacientes" onClick={handleNavigation} className="flex items-center gap-2 hover:opacity-80 transition">
                    <img src={logo} alt="Logo Detecta" className="h-10 md:h-12 w-auto object-contain" />
                </Link>
            </div>
                ...
                <ul className="flex gap-8 list-none">
                    ...
                    return (
                        ...
                        <Link
                            ...
                            ${isActive ? "text-cyan-600 border-cyan-600" : "text-gray-500 border-transparent hover:text-cyan-500 hover:border-cyan-200"}`}
                        >
                            {item.label}
                        </Link>
                        </li>
            ...
            <div className="flex items-center justify-end relative">
                ...
                >
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold border transition-all duration-200 shadow-sm text-sm ${isUserMenuOpen ? "bg-cyan-600 text-white border-cyan-600 ring-2 ring-cyan-100" : "bg-cyan-50 text-cyan-700 border-cyan-200 group-hover:bg-cyan-100"}`}>
                ...
                >
                    <ul className="flex flex-col p-4 gap-2">
                        {MENU_ITEMS.map((item) => {
                             ...
                             return (
                                <li key={item.path}>
                                    <Link to={item.path} onClick={handleNavigation} className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive ? "bg-cyan-50 text-cyan-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
                                        {item.label}
                                    </Link>
                                </li>
                             )
      ...
      {(isUserMenuOpen || isMobileMenuOpen) && (
        <div className="fixed inset-0 z-40 bg-black/5 backdrop-blur-[1px]" onClick={() => { setIsUserMenuOpen(false); setIsMobileMenuOpen(false); }} />
      )}
    </>
  );
};